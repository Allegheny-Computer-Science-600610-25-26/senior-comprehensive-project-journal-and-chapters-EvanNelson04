...
```swift
workoutData.entries.forEach { entry in
    if entry.muscleGroup == targetMuscle {
        matchingExercises.append(entry)
    }
}
```
...
Moving forward in the flow chart we can see that this list WorkoutData.entries, fuels the rest of the processes. 
