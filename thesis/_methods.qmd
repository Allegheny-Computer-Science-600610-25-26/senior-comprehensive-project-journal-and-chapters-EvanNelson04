...
## Overview of WorkoutEvaluator

WorkoutEvaluator is a dynamic fitness app designed to help users log, monitor, analyze, and receive evaluations on their workouts in a seamless 
...
The purpose of WorkoutEvaluator is to provide users with free and accessible tools to track and evaluate their workouts, helping them 
...
WorkoutEvaluator is designed for fitness enthusiasts of all levels, from beginners who want to establish consistent workout habits to advanced 
...
The overall architecture of WorkoutEvaluator is designed to provide a seamless, reactive experience for users while maintaining a simple design.
...
all in one environment. For a mobile app like WorkoutEvaluator, Xcode allows for rapid development and testing, letting developers design 
...
To test WorkoutEvaluator on a live device, the following steps were taken:
...
WorkoutEvaluator, where performance efficiency and battery usage are critical during prolonged use.
...
development environment that supports the entire workflow of the WorkoutEvaluator app, from initial design and coding to real-device testing, 
...
This section introduces the data flow and processing pipeline used in the WorkoutEvaluator application. It explains how user-entered workout 
...
In WorkoutEvaluator, data querying does not run continuously. This means that everything becomes powered and runs when the user presses the save
...
pages that take data from this list: History, Progress (charts and evaluation), and Profile. All of these different sections of WorkoutEvaluator
...
(.listRowBackground(Color.clear)) and no default separator (.listRFowSeparator(.hidden)), giving it a clean, card-like appearance. Users can 
...
The trend analysis and evaluation system is the most analytically sophisticated component of WorkoutEvaluator, and represents the
...
If both weight and reps increased, the feedback reflects progressive overload which is the gold standard of strength training. If only
...
This comparison-first approach is justified because session-to-session comparison is the most immediately meaningful signal to a
...
session. Comparing the current session's volume against the previous session's volume gives a more complete view of
...
One of the more analytically complex components of the evaluation system is the momentum metric. Rather than comparing
only the two most recent sessions, momentum is calculated by comparing the average volume across the five most recent
sessions against the average volume of the five sessions before that. This produces a measure of performance
trajectory that is more valuable for the user than a comparison between the last 2 workouts.
...
The implementation computes the slope, and applies it over an indexed sequence
of sessions rather than raw dates, which avoids potential problems caused by irregular training intervals. The predicted weight
...
that is built in recent trend data and presented as a guide. For a user-facing fitness application, a simple, interpretable 
...
0-to-100 range. A score below 40 indicates a manageable training load with adequate recovery capacity; scores
...
By evaluating these metrics and providing the user with this information, WorkoutEvaluator is able to complete a task that no other
mobile fitness applications have been able to complete. Providing careful justification within this section is important to 
understand why and how many of these metrics are being completed, as well as to demonstrate the uniqueness of this project.

The code for this experiment was developed and is published in a public repository at the following 
link: <https://github.com/EvanNelson04/WorkoutEvaluatorExperiment>.
